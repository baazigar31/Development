...
export const newToken= user=>{
    return jwt.sign({id:user.id}, 'jkasjld',{
        expiresIn:'10d'
    })
};
...
export const verifyToken=token=>
     new Promise((resolve,reject)=>{
         jwt.verify(token,'jkasjld',(err,res)=>{
             if(err) reject(err);
     ...
     export const register = async (req, res) => {
        ...
        try {
            ...
            if(!user){
                ...
                return res.status(205).send("User already exist!!");
            }
...
export const protect = async (req, res, next) => {
    ...
    if(!bearer || !bearer.startsWith('Bearer')) return res.status(401).send("Header authorization failed!!");
